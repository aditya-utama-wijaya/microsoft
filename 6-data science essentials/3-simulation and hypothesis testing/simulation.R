comp.ci <- function(vec, quantile = 0.05){
  ## Compute the upper and lower empirical quantiles
  lower <- quantile / 2.0
  upper <- 1.0 - lower
  c(quantile(vec, probs = lower, na.rm = TRUE), quantile(vec, probs = upper, na.rm = TRUE))
}

dist.summary <- function(dist, name){
  ## Function to plot and print a summary of the distribution
  hist(dist, col = 'blue', breaks = 'FD', xlab = name, main = paste('Distribution of ', name))
...
sim.lemonade(100000)
sim.lemonade(100000, mean = 1200, sd = 40, pois = TRUE)
